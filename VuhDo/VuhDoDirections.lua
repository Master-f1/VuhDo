...
local function VUHDO_getUnitDirection(aUnit)
	if WorldMapFrame and WorldMapFrame:IsShown()
		or GetMouseFocus() and not GetMouseFocus():GetName() then
		return nil;
	end
...
function VUHDO_updateDirectionFrame(aButton)
	...
	if not tInfo or tInfo["range"] or (sIsDeadOnly and not tInfo["dead"]) or not tInfo["connected"] or tInfo["isPet"] then
